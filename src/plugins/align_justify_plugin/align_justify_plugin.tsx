...
export function Icon({ onClick }: { onClick: (option: string) => void }) {
	...
	return (
		<div
			...
			onClick={handleClick}
		>
			<img width="24" height="24" src="https://img.icons8.com/android/24/align-justify.png" alt="align-justify" />
		</div>
	);
}


const alignJustifyPlugin: EditorPlugin = {
	applyFormat(before, selected, after, value) {
		return before + `<p style="text-align: justify;">${selected}</p>` + after;
	},
};

export default alignJustifyPlugin;
