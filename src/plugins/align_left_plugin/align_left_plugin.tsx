...
export function Icon({ onClick }: { onClick: (option: string) => void }) {
	...
	return (
		<div
			...
			onClick={handleClick}
		>
			<img width="24" height="24" src="https://img.icons8.com/ios-glyphs/30/align-left.png" alt="align-left" />
		</div>
	);
}


const alignLeftPlugin: EditorPlugin = {
	applyFormat(before, selected, after, value) {
		return before+`<p style="text-align: left;">${selected}</p>`+after;
	},
};
